...
md"""
I remember there were some issues or updates necessary with this function. Let's see
"""
...
- A graph `G` which apparently comes from `LightGraphs.jl` that IIRC it's deprecated, so we should update it
- A dictionary of lenghts `l` in the format `(i, j) => l`
- Number of ants `m`
- Number of iterations `k_max`
- Pheromone exponent ``\alpha``
- Prior exponent ``\beta``
- Evaporation scalar ``\rho``
- Dictionary of prior edge weights ``\eta``
"""

# ╔═╡ 9a84d2cc-8d85-4986-874a-36c700f6cc02
function ACO(G, lengths; m=1000, k_max=100, α=1.0, β=5.0, ρ=0.5,
			 η=Dict((e.src, e.dst) => 1/lengths[(e.src, e.dst)] for e in edges(G)))
...
begin
	...
	lengths = Dict((e.src, e.dst) => rand(myrng, 0.2:0.2:100) for e in edges(g))
end
...
median(values(lengths))

# ╔═╡ 48a64b7c-c7ac-40ea-820d-44dd3e8c4cc4
n * mean(values(lengths))

# ╔═╡ 81ddc496-3415-4e2c-8214-504987dd6fb4
